...
from clustercontrast.utils.faiss_rerank import compute_camera_invariant_jaccard_distance,compute_modal_camera_invariant_jaccard_distance
    ...
    for epoch in range(25):
        with torch.no_grad():
            ...
            def exponential_decay(initial_value, total_epochs, current_epoch):
                decay_factor = (0.5) ** (current_epoch / total_epochs)
                current_value = initial_value * decay_factor
                return current_value


            def exponential_decay_kb(initial_value, total_epochs, current_epoch):
            ...
            eps=exponential_decay(0.6,args.epochs,epoch)
            ...
            k1 = exponential_decay_k1(20, args.epochs-2, epoch, 3)
            print('k1:', k1)
            kb = exponential_decay_kb(args.kb, args.epochs-2, epoch)
            print('kb:', kb)

            rerank_dist_ir = compute_camera_invariant_jaccard_distance(features_ir, k1=k1, kb=kb,file=sorted(dataset_ir.train),search_option=3,camera_num=6)
            pseudo_labels_ir = cluster_ir.fit_predict(rerank_dist_ir)
            rerank_dist_rgb = compute_camera_invariant_jaccard_distance(features_rgb, k1=k1, kb=kb,file=sorted(dataset_rgb.train),search_option=3,camera_num=6)
        ...
        def generate_cluster_features_random(labels, features):
            ...
            for i, label in enumerate(labels):
                ...
                centers[labels[i]].append(features[i])
            centers = [
                torch.stack(centers[idx], dim=0)[np.random.choice(len(centers[idx]))] for idx in
                sorted(centers.keys())
            ]
        ...
        cluster_features_ir = generate_cluster_features_random(pseudo_labels_ir, features_ir)
        cluster_features_rgb = generate_cluster_features_random(pseudo_labels_rgb, features_rgb)
...
def main_worker_stage2(args,log_s1_name,log_s2_name,subdir,update_subdir=None):
    ...
    if update_subdir is not None:
        args.logs_dir = osp.join('logs' + '/' + update_subdir + '/' + log_s2_name)
    else:
    ...
    for epoch in range(25):
        with torch.no_grad():
            ...
            def exponential_decay_increase(initial_value, total_epochs, current_epoch):
                decay_factor = (2) ** (current_epoch / total_epochs)
                current_value = initial_value * decay_factor
                return current_value
            ...
            if epoch<=10:
                ...
                eps = exponential_decay(0.6, 10, epoch)
            else:
                eps = exponential_decay_increase(0.3, 40, epoch-10)


            ir_eps = eps #stage two no adjust
            print('IR Clustering criterion: eps: {:.3f}'.format(ir_eps))
            cluster_ir = DBSCAN(eps=ir_eps, min_samples=4, metric='precomputed', n_jobs=-1)
            rgb_eps = eps #stage two no adjust
            print('RGB Clustering criterion: eps: {:.3f}'.format(rgb_eps))
            cluster_rgb = DBSCAN(eps=rgb_eps, min_samples=4, metric='precomputed', n_jobs=-1)
            ...
            k1 = exponential_decay_k1(30, args.epochs-2, epoch, 2)
            print('k1:', k1)
            kb = exponential_decay_kb(args.kb, args.epochs-2, epoch)
            print('kb:', kb)
        ...
        #####################################
        pseudo_labels_ir = generate_pseudo_labels(pseudo_labels_ir, num_cluster_ir, dataset_ir)
        memory_instance_IR.labels = pseudo_labels_ir.cuda()

        pseudo_labels_rgb = generate_pseudo_labels(pseudo_labels_rgb, num_cluster_rgb, dataset_rgb)
        memory_instance_RGB.labels = pseudo_labels_rgb.cuda()

        memory_instance_IR.momentum = args.momentum
        memory_instance_RGB.momentum = args.momentum
        print('ins_momentum:', memory_instance_IR.momentum)

        trainer.memory_instance_ir = memory_instance_IR
        trainer.memory_instance_rgb = memory_instance_RGB
...
def main_worker_stage3(args,log_s1_name,log_s2_name,subdir,update_subdir=None):
    start_epoch=0
    best_mAP=0
    # logs_dir_stage1=args.logs_dir
    if update_subdir is not None:
    ...
    for epoch in range(50):
        with torch.no_grad():
            ...
            k1 = exponential_decay_k1(30, args.epochs-2, epoch, 2)
            print('k1:', k1)
            k1m = exponential_decay_k1(40, args.epochs-2, epoch, 3)
            print('k1m:', k1m)
            kb = exponential_decay_kb(args.kb, args.epochs-2, epoch, 3)
            print('kb:', kb)
            kbm = exponential_decay_kb(args.kbm, args.epochs-2, epoch, 1)
            print('kbm:', kbm)
        ...
        trainer.memory_instance_ir = memory_instance_IR
        trainer.memory_instance_rgb = memory_instance_RGB
        trainer.memory_instance_all = memory_instance_ALL
...
if __name__ == '__main__':
    ...
    parser.add_argument('--momentum2', type=float, default=0,
                        help="update momentum2 for the hybrid memory")
