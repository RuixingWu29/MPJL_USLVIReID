...
class firstlayers(nn.Module):
    def __init__(self):
        super(firstlayers, self).__init__()
...
class visible_module(nn.Module):
    def __init__(self, arch='resnet50'):
        ...
        # model_layers=list(model_v.children())
        # # avg pooling to global pooling
        # visible_module_layers=[]
        # # self.visible = model_v
        # for i, layer in enumerate(model_layers):
        #     if i<=3:
        #         print(i,layer)
        #
        #         visible_module_layers.append(layer)
        #         print('visible_module_layers',visible_module_layers)


        visible_new_model=firstlayers()
        pretrained_dict=torch.load(model_urls['resnet50'])
        new_state_dict = {}

        for name, param in visible_new_model.state_dict().items():
            if name in pretrained_dict:
                new_state_dict[name]=pretrained_dict[name]
        visible_new_model.load_state_dict(new_state_dict)
        self.visible = visible_new_model
    def forward(self, x):
