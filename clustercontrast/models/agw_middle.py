...
class embed_net_ori(nn.Module):
    ...
    def forward(self, x1, x2, modal=0,label_1=None,label_2=None):
        ...
        if modal == 0:
            ...
            x = torch.cat((x1, x2), 0)
        elif modal == 4: ##middle modal vis
            gray1 = F.relu(self.encode1(x1))
            gray1 = self.bn1(F.relu(self.fc1(gray1)))
            gray1 = F.relu(self.decode(gray1))

            x = self.visible_module(gray1)
        elif modal == 5: ##middle modal nir
            gray2 = F.relu(self.encode2(x2))
            gray2 = self.bn2(F.relu(self.fc2(gray2)))
            gray2 = F.relu(self.decode(gray2))

            x = self.thermal_module(gray2)
...
def agw_middle(pretrained=False,no_local= 'down', **kwargs):
