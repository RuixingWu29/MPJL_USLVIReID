...
#intra-modality
def compute_camera_invariant_jaccard_distance(target_features,file, k1=20, kb=6, print_flag=True, search_option=0, use_float16=False,camera_num=9):
    end = time.time()
    all_file_name=[]
    camera_id=[]
    ...
    print('camera invariant expension')
    if kb != 1:
        kp = kb*2
        print('kp:', kp)
        ...
        for i in range(N):
            array = np.zeros(9)
            feas_camera_temp = [[] for _ in range(camera_num)]

            for ii in initial_rank[i,:kp]:
                camera_label = camera_id[ii]
                if array[camera_label] < kb:
                    array[camera_label]=array[camera_label]+1
                    feas_camera_temp[camera_label].append(V[ii, :])

            del array

            average_matices = []
            arrays = [np.array(arr) for arr in feas_camera_temp]
            ...
            for arr in arrays:
                if len(arr) > 0:
                    ...
                    if non_empty_rows:
                        ...
                        average_matices.append(average_matrix)

            V_qe[i, :] = np.mean(average_matices, axis=0)
...
#inter-modality
def compute_modal_camera_invariant_jaccard_distance(target_features, file, k1=20, kb=6, print_flag=True, search_option=0, use_float16=False,camera_num=9):
    end = time.time()
    all_file_name = []
    camera_id=[]
    for i, (fname, _, cid) in enumerate(file):
    ...
    if kb != 1:
        ...
        for i in range(N):
            ...
            array_ir = np.zeros(9)
            array_rgb = np.zeros(9)

            for ii in initial_rank[i, :kp]:
                camera_label = camera_id[ii]
                filename=all_file_name[ii]

                if 'ir_modify' in filename and array_ir[camera_label] < kb:
                    array_ir[camera_label]=array_ir[camera_label]+1
                    feas_camera_ir_temp[camera_label].append(V[ii, :])

                if 'rgb_modify' in filename and array_rgb[camera_label] < kb:
                    array_rgb[camera_label] = array_rgb[camera_label] + 1
                    feas_camera_rgb_temp[camera_label].append(V[ii, :])

            del array_ir, array_rgb
